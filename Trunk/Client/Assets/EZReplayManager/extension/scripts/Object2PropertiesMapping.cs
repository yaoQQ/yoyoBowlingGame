...
public class Object2PropertiesMapping : ISerializable{
	...
	private Object2PropertiesMapping parentMapping;
	...
	private int lastChangedFrame = -1;
	//first frame where changes where recognized
	private int firstChangedFrame = -1;
	...
	public Object2PropertiesMapping(GameObject go,bool isParent, Object2PropertiesMapping parentMapping, int childNo) {
		...
		if (isParentObj) { // if gameObject is a parent..
			...
			for(int i=0;i<allChildren.Length;i++) {
				...
				if (!EZReplayManager.singleton.gOs2propMappings.ContainsKey(child)) {
					
					if (child!=gameObject)
						EZReplayManager.singleton.gOs2propMappings.Add(child, new Object2PropertiesMapping(child,false,this,i));
	...
	public void prepareObjectForReplay() {
		...
		if (isParentObj) { //if is a parent gameObject mapping 
			...
			Transform[] allChildren = myParentClone.GetComponentsInChildren<Transform>() ;
			for(int i=0;i<allChildren.Length;i++) {
				GameObject child = allChildren[i].gameObject;
				//map children to numbers
				if (i == childNo) {
		...
		gameObjectClone.name = gameObjectClone.GetInstanceID()+"_"+gameObject.GetInstanceID()+"_"+gameObjectClone.name;
	
		// kill all unneccessary scripts on gameObjectClone
		Component[] allComps = gameObjectClone.GetComponentsInChildren<Component>();
		...
		foreach (Component comp in allComps) {
			...
			if (/*comp.gameObject == gameObjectClone 
				&&*/ comp != comp.GetComponent<Transform>() 
	...
	//			&& comp != comp.GetComponent<ParticleRenderer>()
				...
				&& comp != comp.GetComponent("FlareLayer")
						...
						for (int i=0;i<EZReplayManager.singleton.componentsAndScriptsToKeepAtReplay.Count;i++) {
//							MonoBehaviour.print(EZReplayManager.singleton.componentsAndScriptsToKeepAtReplay[i]);
							if (comp == comp.GetComponent(EZReplayManager.singleton.componentsAndScriptsToKeepAtReplay[i])) {
								found = true;
								break;
							}
						}
		...
		if (firstChangedFrame > -1 && recorderPosition >= firstChangedFrame )  {
				if (recorderPosition <= lastChangedFrame ) {			
					...
					try {
                     ...
                     SavedState state = getStateAtPos(lastChangedFrame);
                    if (state != null) {
                        state.synchronizeProperties(gameObjectClone);
                    }
				}
				
		} else if (gameObjectClone.active) {
			gameObjectClone.active = false;	
		}
		
		
	}
	
	private SavedState getStateAtPos(int recorderPosition) {
        SavedState state = savedStates[recorderPosition];
        return state;
	}	
