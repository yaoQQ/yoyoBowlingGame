...
CoinObjectPool = {}
local this = CoinObjectPool

local ObjectPoolManager = CS.ObjectPoolManager

function CoinObjectPool:new(o)
    o = o or {}
    setmetatable(o,self)
    self.__index = self
    return o
...
function CoinObjectPool:GetInstance()
...
function this:GetPoolCount()
    return table.nums(TableCoinObjectPool.data)
end
...
function this:InitObjectPool()
    ...
    -- 异步加载并实例化对象
    for _, v in pairs(TableCoinObjectPool.data) do
        EffectManager.Instance:CreateEffect(v.projectName, v.keyName,  function (effectControler)
            ...
            if (resource == nil) then
                Loger.PrintError("抢金币-ObjectPool加载错误")
            else
