require "base:mid/mall/Mid_platform_mall_lands_panel"

PlatformMallLandsView = BaseView:new()
local this = PlatformMallLandsView
this.viewName = "PlatformMallLandsView"

--设置面板特性
this:setViewAttribute(UIViewType.Pop_view, UIViewEnum.Platform_Mall_Lands_View, false)

--设置加载列表
this.loadOrders=
{
	"base:mall/platform_mall_lands_panel",
}
...
function this:addEvent()
	self.main_mid.close_Image:AddEventListener(UIEvent.PointerClick,function ()
		ViewManager.close(UIViewEnum.Platform_Mall_Lands_View)
	end)
...
function this:updateMallEnum(mallEnum)
	...
	elseif mallEnum == PlatformMallView.MallType.Blend then
		...
		local diamondList = {}
		local goldList = {}
		for _, v in ipairs(TableBaseShopExchange.data) do
			if v.src_type == 1 then
				table.insert(diamondList, v)
			elseif v.src_type == 2 then
				table.insert(goldList, v)
			end
		end
		for i = 1, #diamondList do
			table.insert(dataList, diamondList[i])
		end
		for i = 1, #goldList do
			table.insert(dataList, goldList[i])
		end
		this.currMallData = dataList
...
function this.onSetDiamond(go,data,index)
	...
	--printDebug("++++++++++++++++++++++我是table"..table.tostring(data))
	...
	item.go_circle_Button:AddEventListener(UIEvent.PointerClick, function ()
		...
		if m_fromType == 0 then
				...
				succeedCallback = function ()
					ViewManager.open(UIViewEnum.Platform_Mall_Tip_Lands_View,{enum = 3,successData = data})
					m_succeedCallback()
				end
			end
		--充值
		RechargeManager.recharge("0", LoginDataProxy.token, tostring(data.id), tostring(LoginDataProxy.playerId), "0", succeedCallback)
	end)
end
		...
		if data.src_item > userInfo.diamond then--钻石不足
			...
			local fun = function()
				...
				if m_fromType == 0 then
					...
					succeedCallback = function ()
						ViewManager.open(UIViewEnum.Platform_Mall_Tip_Lands_View,{enum = 4,successData = data})
						m_succeedCallback()
					end
				end
				--兑换
...
function this.onSetBlendInfo(go,data,index)
	...
	item.return_btn:AddEventListener(UIEvent.PointerClick,function ()
		...
		if data.src_type == 1 then -- 使用现金兑换钻石
			if userInfo.cash >= data.src_item then
