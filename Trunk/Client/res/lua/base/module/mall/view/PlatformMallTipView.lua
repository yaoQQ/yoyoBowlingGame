require "base:mid/mall/Mid_platform_mall_tip_panel"
require "base:module/mall/data/PlatformMallProxy"

--商城特效界面
PlatformMallTipView = BaseView:new()
local this = PlatformMallTipView
this.viewName = "PlatformMallTipView"

--设置面板特性（界面层级、界面枚举、是否参与界面堆栈）
this:setViewAttribute(UIViewType.Feedback_Tip, UIViewEnum.Platform_Mall_Tip_View, false)

--设置加载列表
this.loadOrders =
{
	"base:mall/platform_mall_tip_panel",
}
...
function this:onClose()	
	...
	GlobalTimeManager.Instance.timerController:RemoveTimer("effectDOScaleTime")
end
...
function this.iconFly()
	...
	GlobalTimeManager.Instance.timerController:AddTimer(
		...
		function()
			...
			iconTrans.localPosition = Vector3(0,155,0)
			...
			if m_isDiamond then 
				move1 = iconTrans:DOLocalMove(Vector3(-315.8,cRectm1.localPosition.y-1,0), 0.5)
			else				
			   -- Rectm2.anchoredPosition = Vector2(0, cRectm2.localPosition.y-1050)
				move1 = iconTrans:DOLocalMove(Vector3(6.8,cRectm1.localPosition.y-1,0), 0.5)
			end	
...
function this:initView()
	self.main_mid.success_gold_Image.transform.localPosition = Vector3(0,149,0)
	...
	self.main_mid.success_diamond_Image.transform.localPosition = Vector3(0,149,0)
